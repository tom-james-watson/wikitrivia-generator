...
fn get_types(
    ...
    let instance_of: Option<Vec<String>> = match item_json["claims"]["P31"].as_array() {
        Some(ids) => Some(
            ids.into_iter()
    ...
    let occupations: Option<Vec<String>> = match item_json["claims"]["P106"].as_array() {
        Some(ids) => Some(
            ids.into_iter()
                .map(|id| return item_label::get(id.as_str().unwrap(), id_label_map, &client))
                .filter(|label_option| return label_option.is_some())
...
fn enough_page_views(year: i64, page_views: usize) -> bool {
...
pub fn process_item_json(
    ...
    let types = get_types(&item_json, id_label_map, client)?;
    ...
    if !enough_page_views(year, page_views) {
        return None;
    }
